...
- **Mobile touch targets and side-by-side fields.** This is a mobile-only app (§1): every interactive control (`Input`, `SelectTrigger`, `Button`, `Checkbox`, ...) needs a real touch target, not shadcn's un-adjusted `h-8` (32px) default — override to a minimum of ~44px (`h-11`) at the call site, the same way a screen already overrides shadcn's color tokens (§2). Whenever two fields sit side by side (`grid-cols-2` rows, e.g. Date/Heure pairs), give each grid/flex item `min-w-0` — grid/flex items default to `min-width: auto`, floored at their content's intrinsic width, so a field with its own such floor (a native `<input type="date">`'s segmented value, for instance) won't shrink to its track and will overlap its sibling on narrow phone widths. Verify both at an actual mobile viewport, not just a resized desktop window.
- **Back navigation stays reachable while scrolling.** Any screen-level header with a back arrow (e.g. `features/convocation/components/BackHeader.tsx`) must be `sticky top-0` (with an opaque background, so content doesn't show through underneath it) rather than sitting in normal flow — a form or list tall enough to scroll must never carry the back control off-screen with it. Mirrors the existing `sticky bottom-0` pattern already used for anchored submit bars.
