...
Ce que la maquette **ne montre pas**, et qui compte autant : aucune boîte de dialogue de création ni de modification, aucun contrôle de suppression, aucun contrôle d'archivage, **aucun statut « à venir »**, aucun rattachement d'équipes ou d'adhésions à la saison, aucun bouton « clôturer la saison », aucune pagination, aucun filtre.
...
Les trois champs de la maquette se logent intégralement dans les colonnes existantes. **Aucun `ALTER TABLE` n'est nécessaire.**
...
- `SeasonRepository` (`domain/repositories/season-repository.ts`) **conserve `findCurrent()` inchangée** — elle est consommée par le scoping des équipes (AC-CD-01) — et gagne `findAll()` (liste administrative) ainsi que `create(...)` et `update(...)`. Une seule interface par ressource, pas de `BackofficeSeasonRepository` séparé (même position que `NewsRepository` dans `web-actus`).
- Deux use cases, `CreateSeasonUseCase` et `UpdateSeasonUseCase` (`domain/usecases/`), fonctions async pures — aucun import React / Supabase / TanStack Query (`CLAUDE.md` §3/§6).
...
- `SeasonRepositoryImpl` écrit `label`, `start_date`, `end_date` et **jamais `season_range`** (colonne générée : l'inclure dans un `insert`/`update` fait échouer la requête).
...
| **Données nominatives** | **Aucune** — une saison est un libellé et deux dates, sans aucun lien vers une personne | Pas d'« export nominatif » au sens du CDC §11.3 ; pas de matière RGPD |
...
- **AC-WS-08** — **Aucune colonne n'est ajoutée à `seasons`** dans cette passe (ni `created_at`, ni `created_by`, ni `updated_at`, ni `updated_by`, ni colonne de statut), et `season_range`, `seasons_no_overlap` et `current_season()` sont inchangés.

**Domaine**

- **AC-WS-09** — `'season:write'` est ajoutée à `domain/policies/actions.ts` et vaut `['admin']` dans `rbac-matrix.ts`. **Aucune autre action, aucun autre rôle n'est ajouté** (`CLAUDE.md` §7) — en particulier `'section:manage'` et `'backoffice:access'` sont inchangées.
...
- **AC-WS-25** — `current_season()`, `isCurrentSeason`, `SeasonRepositoryImpl.findCurrent()`, la politique RLS `teams` d'AC-CD-01 et le scoping des équipes du tableau de bord coach sont **inchangés** en définition et en comportement. Vérifiable en régression.
...
- **AC-WS-31** — Aucune donnée de santé, financière ou nominative n'est lue ou écrite par cette feature.
...
| **PO-WS-03** | **Faut-il journaliser la création et la modification d'une saison ?** Absente du CDC §11.3, mais le rôle Administrateur est borné par « actions sensibles journalisées », et modifier les bornes de la saison en cours change la visibilité des équipes de tous les coachs (§4). Sous-question indissociable : faut-il ajouter `created_at`/`created_by`/`updated_at`/`updated_by`, la table n'ayant **aucune** colonne de traçabilité (§2.1) ? Si oui : depuis le use case, jamais depuis un composant, et quelle rétention ? Même famille que PO-WA-04, avec un enjeu d'intégrité supérieur | Référent RGPD / Bureau | Non pour construire — **à trancher avant mise en production** |
...
- **PO-WS-01 est le seul point ouvert qui touche la mise en page** : les libellés des trois états de statut. Il ne bloque pas la conception — un repli lisible se pose trivialement — mais il doit être posé à la développeuse au moment du rendu plutôt que tranché en silence.
...
| Valeurs initiales | Champs vides | Pré-remplies depuis la ligne sélectionnée (`label`, `startDate` → `toDateInputValue`, `endDate` → idem) — AC-WS-24 |
...
| État | Rendu |
|---|---|
| Soumission en cours | Les trois champs et les deux boutons passent `disabled` ; le bouton de validation affiche « Création… » / « Enregistrement… » — même mécanique que `NewsFormDialog`/`BackofficeLoginPage` |
| **Échec par chevauchement de dates (cas le plus fréquent, §7 de cette spec)** | Le dialogue **reste ouvert**, les trois champs **conservent leur saisie** ; un `Alert`/`AlertDescription` (`variant="destructive"`) apparaît en haut du formulaire avec le message **déjà existant et déjà testé** « Les dates de cette saison chevauchent une saison existante. » (`OverlappingSeasonError` → `mapDomainErrorToUiError`, variante `inline`) — aucun nouveau texte à écrire pour ce cas (AC-WS-16, AC-WS-23) |
...
| Champs Libellé/Début/Fin, boutons | `input.tsx`, `label.tsx`, `button.tsx` | Oui |
| Badge de statut | `badge.tsx` | Oui |
| Message d'erreur (liste et dialogue) | `alert.tsx` | Oui |

Aucune primitive à ajouter pour cette passe (§ Sources, point 6) — différence notable avec `web-actus`, où `dialog`/`table`/`skeleton` restaient à vendre.

### Questions UI — récapitulatif, non bloquantes pour construire

- **PO-WS-01** (seule question qui touche la mise en page, §7) — libellés et couleurs des trois états de statut : repli proposé ci-dessus (« Terminée » neutre / « En cours » vert confirmé / « À venir » ambre), à confirmer avant de figer l'écran.
...
- Un message d'interface reste à écrire pour le cas `start_date > end_date` (AC-WS-10) — `mapDomainErrorToUiError` n'a aujourd'hui de variante que pour le chevauchement. Pas une question de conception, mais un texte manquant à ne pas découvrir en recette.
